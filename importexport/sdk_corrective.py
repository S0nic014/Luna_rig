...
class SDKCorrectiveManager(manager.AbstractManager):
    ...
    def export_pose(self, corrective_component, pose_name, driver):
        ...
        for corr_control in pose_dict.keys():
            pose_dict[corr_control]["driver"] = driver
            pose_dict[corr_control]["driver_value"] = pm.getAttr(pose_dict[corr_control]["driver"])
    ...
    def import_pose(self, corrective_component, pose_name):
        ...
        for corr_control_name in pose_dict.keys():
            if not pm.objExists(corr_control_name):
                ...
                continue
            driver_attr = pose_dict[corr_control_name].pop("driver")
            driver_value = pose_dict[corr_control_name].pop("driver_value", 0.0)
